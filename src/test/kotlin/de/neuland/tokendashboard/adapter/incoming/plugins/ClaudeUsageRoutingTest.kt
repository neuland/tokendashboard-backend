...
class ClaudeUsageRoutingTest :
    FunSpec({
        ...
        val claudeIngestService = mockk<IngestClaudeUsagePort>(relaxed = true)
        ...
        fun testApp(block: suspend io.ktor.server.testing.ApplicationTestBuilder.() -> Unit) =
            testRoutingApp(
                module {
                    ...
                    single<IngestClaudeUsagePort> { claudeIngestService }
        ...
        test("GET /api/usage/claude/v1 returns 200") {
            every {
            ...
            testApp {
                val response = client.get("/api/usage/claude/v1?from=2026-06-01&to=2026-06-15")
                response.status shouldBe HttpStatusCode.OK
            }
        }

        test("GET /api/usage/claude/v1 without from returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/v1?to=2026-06-15")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/v1 without to returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/v1?from=2026-06-01")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/v1 with from and to params passes correct dates") {
            val rangeSlot = slot<DayRange>()
            val customFrom = Day(LocalDate.of(2026, 5, 1))
            val customTo = Day(LocalDate.of(2026, 5, 31))
            every { providerUsageQueryService.queryUsage(CLAUDE, capture(rangeSlot)) } returns
                emptyOverview(DayRange.of(customFrom, customTo))

            testApp {
                client.get("/api/usage/claude/v1?from=2026-05-01&to=2026-05-31")
                rangeSlot.captured.from shouldBe customFrom
                rangeSlot.captured.to shouldBe customTo
            }
        }

        test("GET /api/usage/claude/v1 response contains from and to as formatted dates") {
            every {
                providerUsageQueryService.queryUsage(CLAUDE, any())
            } returns emptyOverview(DayRange.of(firstOfMonth, today))

            testApp {
                val body = client.get("/api/usage/claude/v1?from=2026-06-01&to=2026-06-15").bodyAsText()
                body shouldContain "2026-06-01"
                body shouldContain "2026-06-15"
            }
        }

        test("GET /api/usage/claude/v1 response contains model data") {
            every { providerUsageQueryService.queryUsage(CLAUDE, any()) } returns
        ...
        test("GET /api/usage/claude/v1 with malformed from param returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/v1?from=not-a-date&to=2026-06-15")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/series/v1 returns 200") {
            every { providerUsageSeriesQueryService.querySeries(CLAUDE, any(), any()) } returns
                emptySeries(DayRange.of(firstOfMonth, today))

            testApp {
                val response = client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15")
                response.status shouldBe HttpStatusCode.OK
            }
        }

        test("GET /api/usage/claude/series/v1 without from returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/series/v1?to=2026-06-15")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/series/v1 without to returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/series/v1?from=2026-06-01")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/series/v1 response contains history, buckets, model data and yyyy-MM-dd dates") {
            every { providerUsageSeriesQueryService.querySeries(CLAUDE, any(), any()) } returns
                seriesWithModel(DayRange.of(firstOfMonth, today))

            testApp {
                val body = client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15").bodyAsText()
        ...
        test("GET /api/usage/claude/series/v1 without granularity defaults to day") {
            testApp {
                client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15")

                verify { providerUsageSeriesQueryService.querySeries(CLAUDE, dayRange, Granularity.DAY) }
            }
        }

        test("GET /api/usage/claude/series/v1 with granularity=week passes week") {
            testApp {
                client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15&granularity=week")

                verify { providerUsageSeriesQueryService.querySeries(CLAUDE, dayRange, Granularity.WEEK) }
            }
        }

        test("GET /api/usage/claude/series/v1 with granularity=day passes day") {
            testApp {
                client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15&granularity=day")

                verify { providerUsageSeriesQueryService.querySeries(CLAUDE, dayRange, Granularity.DAY) }
            }
        }

        test("GET /api/usage/claude/series/v1 with granularity=month returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/series/v1?from=2026-06-01&to=2026-06-15&granularity=month")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/series/v1 with range within 100 days (default granularity) returns 200") {
            every { providerUsageSeriesQueryService.querySeries(CLAUDE, any(), any()) } returns
                emptySeries(
                    DayRange.of(
            ...
            testApp {
                val response = client.get("/api/usage/claude/series/v1?from=2026-01-01&to=2026-04-11")
                response.status shouldBe HttpStatusCode.OK
            }
        }

        test("GET /api/usage/claude/v1 with from after to returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/v1?from=2026-06-30&to=2026-06-01")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/v1 with 5 years with 2 leap days returns OK") {
            testApp {
                val response = client.get("/api/usage/claude/v1?from=2019-04-01&to=2024-03-31")
                response.status shouldBe HttpStatusCode.OK
            }
        }

        test("GET /api/usage/claude/v1 with a range beyond the maximum returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/v1?from=2019-04-01&to=2024-05-01")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("GET /api/usage/claude/series/v1 with a range beyond the maximum returns 400") {
            testApp {
                val response = client.get("/api/usage/claude/series/v1?from=0001-01-01&to=9999-12-31")
                response.status shouldBe HttpStatusCode.BadRequest
            }
        }

        test("POST /api/usage/ingest/claude with a body beyond the limit returns 413") {
            testApp {
